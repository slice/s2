...
class Roster:
    ...
    def pick_mafia(
        self, *, portion: int = 3, max: int = 2
    ) -> typing.Set[discord.Member]:
...
class MafiaGame:
    ...
    async def pick_mafia(self):
        ...
        overwrites.update({mafia_member: allow for mafia_member in self.roster.mafia})

        self.mafia_chat = await self.guild.create_text_channel(
            generate_name("mafia"), overwrites=overwrites
        )

        flavor = random.choice(["Say hello!", "Greet each other, mafia!",])
    ...
    async def pick_victim(self):
        def check(message):
            return (
                message.channel == self.mafia_chat
                and message.author in self.roster.mafia
            )
    ...
    async def alltalk(self):
        ...
        for member in self.roster.all:
            self.log.debug(
                "alltalk: allowing %r to speak in %r", member, self.game_channel
            )
    ...
    async def gather_hanging_votes(self):
        def check(message):
            return message.channel == self.game_channel and self.roster.is_alive(
                message.author
            )
    ...
    async def game_loop(self):
        ...
        await self.game_channel.send(
            f"{mentions}: The main game will be conducted here! Make sure to have fun!"
        )
        ...
        while True:
            ...
            emoji = (
                "\N{BLACK SUN WITH RAYS}" if self.daytime else "\N{NIGHT WITH STARS}"
            )
            await self.game_channel.send(
                f'{emoji} **{"Day" if self.daytime else "Night"} {self.day}** {emoji}'
            )
            self.log.info(
                ">> Time progression. Day %d, %s.",
                self.day,
                "day" if self.daytime else "night",
            )
            ...
            if self.daytime:
                if self.victim_tonight:
                    ...
                    await self.game_channel.set_permissions(
                        self.victim_tonight, read_messages=True, send_messages=False
                    )
                    await self.game_channel.send(
                ...
                self.log.info(
                    "It is now discussion time. (%d votes required for hanging.)",
                    votes_required,
                )
                await self.game_channel.send(
                ...
                self.hanging_votes = {
                    target_id: len(votes)
                    for target_id, votes in self.hanging_votes.items()
                }
                self.log.info("Hanging votes (postprocessed): %s", self.hanging_votes)
                sorted_votes = sorted(
                    list(self.hanging_votes.items()), key=lambda e: e[1], reverse=True
                )
                vote_board = list(
                    filter(lambda e: e[1] >= votes_required, sorted_votes)
                )
                self.log.info("Final voting board: %s", vote_board)
                if not vote_board:
                    await self.game_channel.send(
                        "A verdict was not reached in time. Oh well!"
                    )
                else:
                    hanged = discord.utils.get(self.roster.players, id=vote_board[0][0])
                    await self.lock()
                    await self.game_channel.set_permissions(
                        hanged, read_messages=True, send_messages=True
                    )
                    await self.game_channel.send(
                    ...
                    await self.game_channel.set_permissions(
                        hanged, read_messages=True, send_messages=False
                    )
                    await self.unlock()
                    await self.game_channel.send(
                        f"\N{SKULL} **Rest in peace, {hanged}. You will be missed.** \N{SKULL}"
                    )
                    ...
                    await self.game_channel.send(
                        f'{hanged} was **{"mafia" if was_mafia else "innocent"}.**'
                    )
    ...
    async def start(self):
        self.log.debug(
            "game started! gathering players (%d needed).", self.REQUIRED_PLAYERS
        )
        ...
        await self.game_channel.send(
            "Game over! This channel will self destruct in 10 seconds."
        )
